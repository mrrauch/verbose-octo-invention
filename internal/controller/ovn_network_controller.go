...
func (r *OVNNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	nbName := fmt.Sprintf("%s-nb-db", instance.Name)
	nbReplicas := int32(1)
	if instance.Spec.NorthboundDBReplicas != nil {
		nbReplicas = *instance.Spec.NorthboundDBReplicas
	}
	if err := r.ensureOVNDB(ctx, instance, nbName, 6641, images.DefaultOVNNBDB, nbReplicas); err != nil {
		return ctrl.Result{}, err
	}

	// Ensure SB DB (StatefulSet + headless Service)
	sbName := fmt.Sprintf("%s-sb-db", instance.Name)
	sbReplicas := int32(1)
	if instance.Spec.SouthboundDBReplicas != nil {
		sbReplicas = *instance.Spec.SouthboundDBReplicas
	}
	if err := r.ensureOVNDB(ctx, instance, sbName, 6642, images.DefaultOVNSBDB, sbReplicas); err != nil {
		return ctrl.Result{}, err
	}
	...
	instance.Status.NorthboundDBEndpoint = fmt.Sprintf("tcp:%s.%s.svc:6641", nbName, instance.Namespace)
	instance.Status.SouthboundDBEndpoint = fmt.Sprintf("tcp:%s.%s.svc:6642", sbName, instance.Namespace)

	// Check if NB DB StatefulSet is ready
	nbSts := &appsv1.StatefulSet{}
	if err := r.Get(ctx, types.NamespacedName{Name: nbName, Namespace: instance.Namespace}, nbSts); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *OVNNetworkReconciler) ensureHeadlessService(ctx context.Context, instance *openstackv1alpha1.OVNNetwork, name string, port int32) error {
	...
	_, err := controllerutil.CreateOrUpdate(ctx, r.Client, svc, func() error {
		svc.Labels = labelsForOVN(instance.Name, component)
		svc.Spec.ClusterIP = corev1.ClusterIPNone
		svc.Spec.Selector = labelsForOVN(instance.Name, component)
...
func (r *OVNNetworkReconciler) ensureDBStatefulSet(ctx context.Context, instance *openstackv1alpha1.OVNNetwork, name string, port int32, image string, replicas int32) error {
	...
	_, err := controllerutil.CreateOrUpdate(ctx, r.Client, sts, func() error {
		sts.Labels = labelsForOVN(instance.Name, component)
...
func (r *OVNNetworkReconciler) ensureNorthd(ctx context.Context, instance *openstackv1alpha1.OVNNetwork) error {
	name := fmt.Sprintf("%s-northd", instance.Name)
	replicas := int32(1)
	...
	_, err := controllerutil.CreateOrUpdate(ctx, r.Client, dep, func() error {
		dep.Labels = labelsForOVN(instance.Name, "northd")
				...
				Containers: []corev1.Container{
					{
						...
						Env: []corev1.EnvVar{
							{Name: "OVN_NB_DB", Value: fmt.Sprintf("tcp:%s-nb-db.%s.svc:6641", instance.Name, instance.Namespace)},
							{Name: "OVN_SB_DB", Value: fmt.Sprintf("tcp:%s-sb-db.%s.svc:6642", instance.Name, instance.Namespace)},
						},
					},
				},
			},
...
func componentFromName(name string) string {
	switch {
	case strings.HasSuffix(name, "-nb-db"):
		return "nb-db"
	case strings.HasSuffix(name, "-sb-db"):
		return "sb-db"
	default:
		return name
	}
